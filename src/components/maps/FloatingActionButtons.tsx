...
export function FloatingActionButtons({
  ...
  return (
    ...
    >
      ...
      {state.isExpanded && hasContextualActions && (
        <div className="absolute inset-0">
          {state.contextualActions.map((action, index) => {
            const position = actionPositions[index] || { x: 0, y: 0 };
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  state.isAnimating ? 'animate-scale-in' : '',
                )}
                style={{
      ...
      {/* Botão principal */}
      {hasPrimaryAction && state.primaryAction && (
        <Button
          variant={state.primaryAction.primary ? 'default' : 'secondary'}
          size="default"
          className={cn(
            'h-14 w-14 p-0 rounded-full shadow-xl',
            'bg-primary text-primary-foreground',
            'hover:bg-primary/90 hover:shadow-2xl',
            'transform hover:scale-110 active:scale-95',
            'transition-all duration-200 ease-out',
            state.isExpanded && 'rotate-45',
            state.primaryAction.disabled && 'opacity-50 cursor-not-allowed'
          )}
          onClick={() => {
          ...
          aria-label={
            hasContextualActions 
              ? state.isExpanded ? 'Fechar menu' : 'Abrir menu de ações'
        ...
        >
          {hasContextualActions ? (
            ...
            <state.primaryAction.icon className="w-6 h-6" />
          )}
        </Button>
      )}
      ...
      {!state.isExpanded && hasContextualActions && (
        <div className="absolute -top-1 -right-1 h-4 w-4 bg-destructive text-destructive-foreground rounded-full flex items-center justify-center text-xs font-bold">
      ...
      {state.isExpanded && (
        <div className="absolute top-full left-0 mt-2 p-2 bg-background/90 backdrop-blur-sm border border-border rounded-lg text-xs text-muted-foreground max-w-48">
          <div className="font-medium mb-1">Atalhos:</div>
          {state.allActions
            .filter(action => action.shortcut && !action.disabled)
            .slice(0, 3)
            .map(action => (
              <div key={action.id} className="flex justify-between items-center py-0.5">
