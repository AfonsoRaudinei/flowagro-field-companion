...
export const FlowAgroLogo: React.FC<FlowAgroLogoProps> = ({
  ...
  if (variant === 'icon') {
    return <div className={cn("inline-flex items-center justify-center rounded-lg", sizeClasses[size], className)}>
        
      </div>;
  }
  if (variant === 'wordmark') {
    return <div className={cn("inline-flex items-center font-primary font-bold", sizeClasses[size], className)}>
        <span className="text-current tracking-tight" style={{
        color: colorValue
      }}>
          {FLOWAGRO_BRAND.name}
        </span>
  ...
  return <div className={cn("inline-flex items-center gap-3", sizeClasses[size], className)}>
      {/* Icon part */}
      <div className={cn("inline-flex items-center justify-center rounded-lg", sizeClasses[size])}>
        <svg viewBox="0 0 24 24" fill="none" className="w-full h-full" style={{
        color: colorValue
      }}>
          <path d="M12 2L8 6v4l4-2 4 2V6l-4-4z" fill="currentColor" opacity="0.9" />
          <path d="M8 10v6c0 2.2 1.8 4 4 4s4-1.8 4-4v-6l-4 2-4-2z" fill="currentColor" />
          <circle cx="12" cy="16" r="1.5" fill="white" opacity="0.8" />
        </svg>
      </div>
      
      {/* Wordmark part */}
      <div className={cn("inline-flex items-center font-primary font-bold", sizeClasses[size])}>
        <span className="text-current tracking-tight" style={{
        color: colorValue
      }}>
          {FLOWAGRO_BRAND.name}
...
export const BrandBadge: React.FC<BrandBadgeProps> = ({
  ...
  return <span className={cn('inline-flex items-center rounded-full border font-medium', variantClasses[variant], sizeClasses[size], className)}>
      {children}
    </span>;
};
...
export const BrandCard: React.FC<BrandCardProps> = ({
  ...
  return <div className={cn('rounded-lg overflow-hidden transition-all duration-200', variantClasses[variant], className)}>
      {children}
    </div>;
};
...
export const BrandButton: React.FC<BrandButtonProps> = ({
  ...
  return <button onClick={onClick} disabled={disabled} className={cn('inline-flex items-center justify-center gap-2 rounded-lg font-medium transition-all duration-200', 'hover:-translate-y-0.5 active:scale-98 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2', 'disabled:opacity-50 disabled:pointer-events-none disabled:transform-none', variantClasses[variant], sizeClasses[size], className)}>
      {children}
    </button>;
};