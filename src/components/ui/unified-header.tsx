...
export const UnifiedHeader: React.FC<UnifiedHeaderProps> = (props) => {
  ...
  if (!props.variant || props.variant === 'standard') {
    const { title, onBack, showBackButton = true, rightActions, className } = props;
  ...
  if (props.variant === 'conversation') {
    const { name, subtitle, avatar, isOnline, onBack, onMore, className } = props;
    
    return (
      <header className={cn(
        "flex items-center justify-between h-touch px-base py-sm",
        "bg-card/95 backdrop-blur-md border-b border-border/50 shadow-ios-sm",
        className
      )}>
        {/* Left side - Back button */}
...
export const NavigationHeader: React.FC<StandardHeaderProps> = (props) => (
  <UnifiedHeader {...props} variant="standard" />
);

export const IOSHeader: React.FC<Omit<IOSHeaderProps, 'variant'>> = (props) => (
  <UnifiedHeader {...props} variant="ios" />
);

export const ConversationHeader: React.FC<Omit<ConversationHeaderProps, 'variant' | 'title'> & { name: string }> = (props) => (
  <UnifiedHeader {...props} variant="conversation" title={props.name} />
);

export const CleanHeader: React.FC<Omit<CleanHeaderProps, 'variant'>> = (props) => (
  <UnifiedHeader {...props} variant="clean" />
);