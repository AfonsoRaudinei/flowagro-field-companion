-- Corrigir o formato do UUID e criar dados de demonstração
DO $$
DECLARE
    demo_user_id uuid := 'demo1111-1111-1111-1111-111111111111'::uuid;
BEGIN
...
-- Temporariamente permitir visualização pública para demonstração
-- ATENÇÃO: Remover essas políticas em produção
DROP POLICY IF EXISTS "Demo - Allow public read on producers" ON producers;
DROP POLICY IF EXISTS "Demo - Allow public read on conversations" ON conversations;
DROP POLICY IF EXISTS "Demo - Allow public read on messages" ON messages;
DROP POLICY IF EXISTS "Demo - Allow public read on user_preferences" ON user_preferences;

CREATE POLICY "Demo - Allow public read on producers" ON producers FOR SELECT USING (true);
CREATE POLICY "Demo - Allow public read on conversations" ON conversations FOR SELECT USING (true);
CREATE POLICY "Demo - Allow public read on messages" ON messages FOR SELECT USING (true);
CREATE POLICY "Demo - Allow public read on user_preferences" ON user_preferences FOR SELECT USING (true);